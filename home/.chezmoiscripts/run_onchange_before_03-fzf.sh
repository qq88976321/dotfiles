...
if [[ ! -d ~/.fzf ]]; then
    LATEST_TAG=$(git ls-remote --tags --sort="v:refname" https://github.com/junegunn/fzf.git | tail -n1 | sed 's/.*\///; s/\^{}//')
    git clone --depth 1 --branch "$LATEST_TAG" https://github.com/junegunn/fzf.git ~/.fzf
    ~/.fzf/install --all
else
